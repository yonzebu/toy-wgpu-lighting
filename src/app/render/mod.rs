...
pub struct PointLight {
    /// Expected to be stored in column major order
...
pub trait Render {
    fn render_in_pass<'a>(&'a mut self, pass: &mut wgpu::RenderPass<'a>, info: &PassInfo<'a>);
}
...
//     pub fn update_uniforms<'a>(&mut self, state: &PassInfo<'a>) {
...
// impl ModelObject {
//     pub fn update_uniforms<'a>(&mut self, state: &PerPassState<'a>) {
//         let update_fn = |transform: &Transform| {
//             let mat = transform.to_matrix();
//             state.frame.queue.write_buffer(
//                 &self.uniform,
...
pub struct ShadowedModelInstanced {
...
impl ShadowedModelInstanced {
    ...
    pub fn update_uniforms<'a>(&mut self, info: &PassInfo<'a>) {
        let update_fn = |transforms: &Vec<Transform>| {
            ...
            self.transform_matrices
                ...
                .for_each(|(matrices, transform)| {
                    ...
                    let mv = info.view * m;
                    // let mvp = info.proj * mv;
                    model_mat.copy_from_slice(&m.to_cols_array());
            ...
            info.frame.queue.write_buffer(
                &self.instance_buffer,
                0,
                bytemuck::cast_slice(&self.transform_matrices),
            );
        };
        if info.view_proj_changed {
...
impl Render for ShadowedModelInstanced {
    fn render_in_pass<'a>(&'a mut self, pass: &mut wgpu::RenderPass<'a>, info: &PassInfo<'a>) {
        self.update_uniforms(info);
        if info.index == 0 {
            pass.set_pipeline(&self.shadow_pipeline);
        ...
        for mesh in &self.model.meshes {
            ...
            pass.set_vertex_buffer(0, mesh.vertex_buffer.slice(0..));
            pass.set_vertex_buffer(1, self.instance_buffer.slice(0..));
            // TODO: make this Uint16 by fixing the model loading? or does u32 make more sense anyways?
            pass.set_index_buffer(mesh.index_buffer.slice(0..), wgpu::IndexFormat::Uint32);
            pass.draw_indexed(0..mesh.num_elements, 0, 0..self.transforms.len() as u32);
        }
    }
}

pub enum ShadowedRenderObject {
    // RawVertex(RawVertexObject),
    // RawVertexInstanced(RawVertexInstanced),
    // _Model(ModelObject),
    ModelInstanced(ShadowedModelInstanced),
    _Dynamic(Box<dyn Render>),
}

impl Render for ShadowedRenderObject {
    fn render_in_pass<'a>(&'a mut self, pass: &mut wgpu::RenderPass<'a>, info: &PassInfo<'a>) {
        match self {
            // Self::RawVertex(raw) => raw.render_in_pass(info, info.index),
            // Self::RawVertexInstanced(raw) => raw.render_in_pass(state),
            // Self::_Model(model) => model.render_in_pass(state, pass_index),
            Self::ModelInstanced(model) => model.render_in_pass(pass, info),
            Self::_Dynamic(dynamic) => dynamic.render_in_pass(pass, info),
        }
    }
}
